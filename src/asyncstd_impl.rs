...
impl Server {
    pub async fn new(host: String, password: [u8; 32], message_handler: MessageHandler,workers : usize) -> Result<(), Error> {
        ...
        Server::start(Arc::new(Server { listener, message_handler, aesgcm }),workers).await
    }
            ...
            task::spawn(async move {
                ...
                loop {
                    ...
                    for (index, connection) in connections.iter_mut().enumerate() {
                        ...
                        let interaction_type = if let Ok(result) = timeout(Duration::from_millis(10), async {
                    ...
                    if let Some(index) = current {
                        if let Some(connection) = connections.get_mut(index) {
                            ...
                            let message_size = match timeout(Duration::from_secs(5), async {
                                let mut bytes = [0u8; 8];
                                stream.read_exact(&mut bytes).await.map(|_| u64::from_be_bytes(bytes) as usize)
                            }).await {
                                Ok(Ok(size)) => size,
                                _ => {
                                    delete.push(index);
                            ...
                            if let Ok(encrypted) = server.aesgcm.encrypt(&GenericArray::from_slice(&nonce), response.as_slice()) {
                                ...
                                if let Err(_) = stream.write_all(&response_payload).await {
                                    delete.push(index);
                                    connection.2 = false;
                                }
                            }
...
impl Client {
    ...
    pub async fn message(&mut self, bytes: Vec<u8>) -> Result<Vec<u8>, Error> {
        ...
        let encrypted = match self.aesgcm.encrypt(GenericArray::from_slice(&nonce), Payload::from(bytes.as_slice())) {
            Ok(enc) => enc,
            Err(e) => return Err(Error::new(ErrorKind::Other, e.to_string()))
        };
