...
impl Server{
    ...
    pub async fn start(self : Arc<Server>,workers : usize) -> Result<(),std::io::Error>{
        ...
        for _ in 0..workers{
            let w: (Sender<TcpStream>, Receiver<TcpStream>) = channel(1);
            ...
            tokio::task::spawn(async move {
                ...
                loop{
                    if let Some(stream) = receiver.recv().await{
                        let num = {
                    ...
                    while let Some ((index,connection)) = iter.next(){
                        ...
                        if let Ok(dur) = connection_health.get(&connection.1).unwrap().elapsed(){
                            ...
                            if timeout(Duration::from_secs(1), async move {
                                if let Err(e) = stream.read(&mut interaction_type).await{
                                    return Err(e);
                                }
                                return Ok(());
                            }).await.is_err(){continue;}else {
                    ...
                    if let Some(index) = current{
                        if let Some(a) = connections.get_mut(index){
                            let stream = &mut a.0;
                            let message_size = {
                                let mut bytes = [0u8;8];
                                if let Err(_) = stream.read(&mut bytes).await{
                                    continue;
                                }
                            ...
                            let payload = {
                                ...
                                if stream.read(&mut bytes).await.is_err(){
                                    continue;
                                };
                                if bytes.len() != message_size{

                                    let _ = stream.shutdown();
                                    let mut tuple = connections.remove(index);
                                    let _ = tuple.0.shutdown();
                    ...
                    if to_delete{
                        for i in delete{
                            let mut tuple = connections.remove(i);
                            let _ = tuple.0.shutdown().await;
                            connection_health.remove(&tuple.1);
                            cc_changed = true;
                        }
        ...
        loop {
            ...
            if let Err(_) = stream.read(&mut request_type_buffer).await{
            ...
            if RequestType::Authentication == request_type{
                ...
                if let Err(_) = stream.read(&mut password).await{
